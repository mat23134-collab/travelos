...
- cityTransport (mandatory): practical mobility for the DESTINATION city — NOT flight advice
  • intro: max 35 words (apps, payment culture, what is walkable vs transit)
  • options: 4–6 objects; mode = real local name (Metro line family, S-Bahn, city bus network, bike share, ferry, typical taxi/Uber context); summary max 22 words
  • EVERY option MUST include: dailyAverage (string — realistic spend for ONE day using that mode, local currency) AND tripTotalEstimate (string — rough total for the ENTIRE trip day-count you are given in CITY_TRANSPORT_TRIP_DAYS, same currency, clearly labeled e.g. "~€X for N days"); typicalPrice optional short combined band
  • Each option MAY include optionUrl (https only, official pass/tickets) + optionLinkLabel when confident; else omit
  • tip optional max 12 words
  • links: 2–5 objects; label + description in trip language; url MUST be https and point to official transit authority, city transport union, national rail booking, or a single well-known city pass site — NEVER invent domains; if no safe URL, omit that link entry entirely
...

{
  ...
  "cityTransport": {
    ...
    "links": [
      { "label": "Official or major vendor name", "url": "https://… real HTTPS only — official transit, city pass, or national rail booking when relevant", "description": "max 14 words — what you can buy there" }
    ]
  },
  "basecamp": {
    "type": "booked",
    ...
    "recommendations": [
      {
        ...
        "otaPriceCompare": [
          { "source": "Booking.com", "indicativeNightly": "€265–€310/night when rate known, else null", "note": "Write exactly SOLD OUT for these dates ONLY when HOTEL_SEARCH_DATA explicitly shows this property has no rooms. If availability is simply unknown (not in HOTEL_SEARCH_DATA), leave note null — never guess SOLD OUT." },
          { "source": "Agoda", "indicativeNightly": "€265–€310/night when rate known, else null", "note": "Write exactly SOLD OUT for these dates ONLY when HOTEL_SEARCH_DATA explicitly shows this property has no rooms. If availability is simply unknown (not in HOTEL_SEARCH_DATA), leave note null — never guess SOLD OUT." },
...
- If no hotel: set basecamp.type="recommendations", omit booked{}, provide exactly 3 hotel recommendations in recommendations[] based on HOTEL_SEARCH_DATA (if available) or expertise. Each must include ALL keys shown in the sample recommendations[] object:
  • DATE-FIRST AVAILABILITY FILTER (STRICT): When TRIP_HOTEL_DATES is present, recommend a hotel ONLY when HOTEL_SEARCH_DATA contains that exact property WITH a numeric nightly rate for those dates. NEVER recommend a famous/landmark hotel from your own memory if it is not present in HOTEL_SEARCH_DATA — doing so risks sending the user to a sold-out page. If a property appears sold out/no rooms/no availability, set availability=false, write SOLD OUT in availabilitySummary, and mark the Booking.com/Agoda note as exactly SOLD OUT for these dates. Always prefer replacing sold-out or memory-only properties with an available property that actually appears in HOTEL_SEARCH_DATA.
  ...
  • otaPriceCompare: mandatory — exactly 3 rows in this fixed order: Booking.com, Agoda, Airbnb. Booking.com and Agoda MUST include a numeric indicativeNightly only when HOTEL_SEARCH_DATA indicates rooms/rates for TRIP_HOTEL_DATES. CRITICAL: only mark a row note="SOLD OUT for these dates" when HOTEL_SEARCH_DATA EXPLICITLY shows that specific property has no rooms for those dates. HOTEL_SEARCH_DATA usually contains live inventory from ONLY ONE OTA — for the other OTAs availability is UNKNOWN, NOT sold out: leave their note=null (the UI turns them into a "verify live" search link so the user can check real availability). NEVER write SOLD OUT just because an OTA is absent from HOTEL_SEARCH_DATA. Do not label unknown availability as available.
  ...
  • availability: true ONLY when Booking.com or Agoda rows have a nightly rate / available-room evidence for TRIP_HOTEL_DATES. false when sold out. null when unknown.
...
- cityTransport: every options[] object MUST include dailyAverage + tripTotalEstimate (see CITY_TRANSPORT_TRIP_DAYS)
`;
}
