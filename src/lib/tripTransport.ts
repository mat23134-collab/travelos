...
// ── City meta (transport guide + cached tips) ─────────────────────────────────────────────

export interface CityMeta {
  transportGuide: CityTransportGuide | null;
  packingTips: string[] | null;
  localTips: string[] | null;
}

/**
 * Single-query fetch of everything we might have cached for a city:
 * the transport guide, packing tips, and local tips.
 * Returns nulls for any fields not yet populated.
 */
export async function fetchCityMeta(
  db: SupabaseClient,
  city: string,
): Promise<CityMeta> {
  const key = normalizeCityKey(city);
  if (!key) return { transportGuide: null, packingTips: null, localTips: null };

  const { data, error } = await db
    .from('transportation')
    .select('guide, packing_tips, local_tips')
    .eq('city_norm', key)
    .maybeSingle();

  if (error) {
    console.warn('[tripTransport] fetchCityMeta failed (non-critical):', error.message);
    return { transportGuide: null, packingTips: null, localTips: null };
  }

  return {
    transportGuide: parseTransportGuideJson(data?.guide),
    packingTips:    Array.isArray(data?.packing_tips) ? (data.packing_tips as string[]) : null,
    localTips:      Array.isArray(data?.local_tips)   ? (data.local_tips   as string[]) : null,
  };
}

/**
 * Write packing/local tips back to the transportation row after the AI generates
 * them for the first time. Uses upsert so it creates the row if the transport
 * scout hasn't run yet (the guide column stays null until the scout fills it in).
 */
export async function upsertCityTips(
  db: SupabaseClient,
  city: string,
  tips: { packingTips?: string[]; localTips?: string[] },
): Promise<void> {
  const city_name = city.trim();
  if (!city_name) return;
  if (!tips.packingTips?.length && !tips.localTips?.length) return;

  const row: Record<string, unknown> = { city_name };
  if (tips.packingTips?.length) row.packing_tips = tips.packingTips;
  if (tips.localTips?.length)   row.local_tips   = tips.localTips;

  const { error } = await db
    .from('transportation')
    .upsert(row, { onConflict: 'city_norm' });

  if (error) {
    console.warn('[tripTransport] upsertCityTips failed (non-critical):', error.message);
  } else {
    console.log('[tripTransport] city tips cached for:', city_name);
  }
}
