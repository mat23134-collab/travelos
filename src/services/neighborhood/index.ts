 ...
 * works for EVERY city. It is CITY-GENERIC (not per-traveler): the same intro is
 * built once per city and reused for every trip and traveler, keyed by city
 * alone (see buildCityGuideCacheKey). Reuses the same Tavily + Exa + Gemini
 * grounding and the same NeighborhoodProfile shape so the existing guide
